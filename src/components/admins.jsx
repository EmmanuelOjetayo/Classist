...
const downloadVerifiedPDF = async () => {
  ...
  try {
    ...
    const doc = new jsPDF();
    const campFee = 4000;
    const totalAmount = res.documents.length * campFee;
    ...
    doc.rect(pageWidth - 75, 40, 60, 20, 'F');
    doc.setTextColor(20, 158, 136);
    doc.setFontSize(8);
    doc.text("TOTAL REVENUE", pageWidth - 70, 46);
    doc.setFontSize(12);
    doc.text(`N${totalAmount.toLocaleString()}`, pageWidth - 70, 54);

    // --- 4. DATA TABLE ---
    const tableData = res.documents.map((s, i) => [
      i + 1, 
      s.name.toUpperCase(), 
      s.matric, 
      `N${campFee.toLocaleString()}`, 
      "VERIFIED"
    ]);

    autoTable(doc, {
      startY: 65,
      head: [['S/N', 'STUDENT NAME', 'MATRIC NO', 'AMOUNT', 'STATUS']],
      body: tableData,
      theme: 'grid',
      headStyles: { 
    ...
    {selectedStudents.length > 0 && (
      <div className="bg-teal-900 rounded-[2.5rem] p-8 text-white shadow-2xl">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8">
          ...
          <button 
            onClick={() => downloadVerifiedPDF("GST101")} // Replace with dynamic code
